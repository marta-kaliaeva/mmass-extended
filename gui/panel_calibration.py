...
class panelCalibration(wx.MiniFrame):
    ...
    def makeToolbar(self):
        ...
        self.cubicFit_radio = wx.RadioButton(panel, -1, "Cubic")
        self.cubicFit_radio.SetFont(wx.SMALL_FONT)
        self.cubicFit_radio.SetValue((config.calibration['fitting'] == 'cubic'))
        self.cubicFit_radio.Bind(wx.EVT_RADIOBUTTON, self.onModelChanged)
        ...
        sizer.Add(self.cubicFit_radio, 0, wx.ALIGN_CENTER_VERTICAL)
        sizer.AddSpacer(20)
    ...
    def makeReferencesList(self):
        ...
        # init list
        self.referencesList = mwx.sortListCtrl(self, -1, size=(750, 250), style=mwx.LISTCTRL_STYLE_SINGLE)
        ...
        self.referencesList.InsertColumn(4, "Error before (ppm)", wx.LIST_FORMAT_RIGHT)
        self.referencesList.InsertColumn(5, "Error after (ppm)", wx.LIST_FORMAT_RIGHT)
        
        # set column widths
        for col, width in enumerate((180,90,90,90,110,110)):
    ...
    def onModelChanged(self, evt):
        ...
        if self.cubicFit_radio.GetValue():
            config.calibration['fitting'] = 'cubic'
